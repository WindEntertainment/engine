...
const formatExecutable = resolve(__dirname + '/format');
...
connection.onDidChangeConfiguration((change) => {
  if (hasConfigurationCapability) {
    ...
    globalSettings = change.settings.wi || defaultSettings;
  }
...
documents.onDidSave((change) => {
  ...
  const formatProcess = spawn(formatExecutable);
  // const formatProcess = spawn(formatExecutable, ['arg1', 'arg2']);
  ...
  formatProcess.on('close', (code) => {
    console.log(`format process exited with code ${code}`);
  });
  // validateTextDocument(change.document);
});
