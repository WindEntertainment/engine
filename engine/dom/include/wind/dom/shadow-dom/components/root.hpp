...
#include "pre-element.hpp"

namespace wind::dom::shadow {
  class Root : public UIElement<Root, dom::Root, attributes::Root> {
  public:
    Root();
    Root(const unsigned int id);
    ...
    // shadow::ElementPtr
    // findElementById(const shadow::ElementPtr root, const unsigned int& id) {
    std::shared_ptr<shadow::Root> findElementById(
      const std::shared_ptr<shadow::Root> root,
