  ...
  namespace assets {
    ...
    void AssetBundler::build(const fs::path &_path) {
      ...
      try {
        ...
        spdlog::error(
          "Failed create directory iterator for {} directory: {}",
          cachePath.string(),
          ex.what()
        );
        return;
    ...
    AssetBundler::createRecursiveIterator(const fs::path &_path) {
      if (!fs::exists(_path))
        throw AssetBundlerError(
          "Cannot create recursive directory iterator by specified path {} as "
          "it's a non exists location",
          _path.string()
        );
