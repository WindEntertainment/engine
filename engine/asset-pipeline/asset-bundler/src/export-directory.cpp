...
namespace wind {
  namespace assets {
    ...
    void AssetBundler::exportDirectory(
      ...
      spdlog::info(
        "Export directory {} to {}", _source.string(), _destination.string()
      );
      ...
      for (const auto &entry : it) {
        ...
        try {
          spdlog::info(
            "Export file: {} to {}", name.string(), destination.string()
          );
          fs::copy_file(
