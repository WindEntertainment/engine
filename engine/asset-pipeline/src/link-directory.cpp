...
namespace wind {
  ...
  namespace assets {
    ...
    void AssetBundler::linkDirectory(
      ...
      if (!bundle.is_open()) {
        spdlog::error(
          "Cannot create file for create bundle by path {}",
          _destination.string()
        );
        return;
      }
      ...
      for (const auto& entry : it) {
        ...
        if (!file.is_open()) {
          spdlog::warn(
            "Cannot read entry in directory {}", entry.path().string()
          );
          continue;
        }

        files.insert(std::make_pair(
          fs::relative(entry.path(), _source).replace_extension(),
          std::move(file)
        ));
      }

      asset_id header_size =
        files.size() * sizeof(asset_id) * 2 + sizeof(asset_id);
      ...
      for (auto& pair : files) {
        ...
        spdlog::info(
          "Write meta-resource. name: '{}', id: {}, offset: {}",
          pair.first.string(),
          id,
          offset
        );
